...
test("recent project keeps the newest session cwd separate from canonical identity", () => {
  const canonicalRoot = "/repo/packages/app";
  const linkedCwd = "/repo-worktrees/feature/packages/app";
  const item = {
    ...session("linked", canonicalRoot, "2026-08-13T00:00:00.000Z"),
    cwd: linkedCwd,
  };

  assert.deepEqual(getRecentProjects([item]), [{
    key: item.projectKey,
    root: linkedCwd,
  }]);
});
